...
- **The debug switch.** `?debug=1` toggles extra output that exposes server internals appropriate to the bug, the exact SQL and returned rows (SQLi), the admin's rendered page (XSS), or the disassembly, symbol addresses, `checksec`, received-bytes hexdump, crash registers, and a live stack-frame table (binary). It is sticky: every page computes `$debugSuffix` and appends it to internal links, and an inline `onchange` handler rewrites the URL's `debug` param. Debug-only CodeMirror bundles are `<script>`-included only when `debug=1`.
...
Rebuild the base chain whenever you touch anything in `platform/` (editor sources, linters, `fix.php`, `lint.php`, `psalm.xml`, `nbuild`, `native-run.php`, any Dockerfile), challenge images layer `FROM ${BASE_IMAGE}` (default `glassbox-php` or `glassbox-native`) on top of it. **A stale podman COPY layer can silently ship the old `fix.php`/bundle;** if a `platform/` edit doesn't seem to take effect, rebuild the affected base image with `--no-cache`.
...
- **Plain punctuation in prose.** Do not use em dashes (the `, ` character) or a bare `---` anywhere in human text (READMEs, solutions, page copy, code comments). Use commas, colons, parentheses, or separate sentences instead.
