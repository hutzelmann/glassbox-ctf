...
challenges/
  ...
  binary/           binary-exploitation challenges (x86-64 binary, multi-arch container)
...
Binary challenges ship a single fixed-architecture binary (x86-64 today, pinned by `nbuild --arch` in `build.sh`), but their **containers are multi-arch**: the image publishes for `linux/amd64` and `linux/arm64` like every other challenge, so the plain `docker run` is native on any host with no `--platform` flag, and every learner faces the same binary and the same walkthrough regardless of host. The runner executes the binary as a subprocess (`proc_open`, sandboxed with `timeout`/`ulimit`); when the host architecture does not match the binary it wraps it with `qemu-user` (explicitly, never via `binfmt`) and points `QEMU_LD_PREFIX` at the guest sysroot so a dynamic binary finds its loader/libc. Static debug introspection (`objdump`/`nm`/readelf) reads the ELF and is arch-independent; the live `gdb` frame capture also works cross-arch (native `gdb`, or `gdb-multiarch` over `qemu`'s gdbstub).
...
Rebuild the base chain whenever you touch anything in `platform/` (editor sources, linters, `fix.php`, `debug.php`, `lint.php`, `psalm.xml`, `nbuild`, `native-run.php`, `gdb-dump.py`, `selftest.sh`, any Dockerfile), challenge images layer `FROM ${BASE_IMAGE}` (default `glassbox-php` or `glassbox-native`) on top of it. **A stale podman COPY layer can silently ship the old `fix.php`/bundle;** if a `platform/` edit doesn't seem to take effect, rebuild the affected base image with `--no-cache`.

CI (`.github/workflows/docker-publish.yml`) **auto-discovers** challenges: a `discover` job scans `challenges/**/Dockerfile`, then a `base` job builds+pushes `harness`, then the families `php` and `native`, then a `challenges` matrix builds+pushes every `glassbox-ctf-<name>` on push to `main`. **Adding a challenge is just adding its folder**: there is no matrix to edit. Each challenge's family base is parsed from its own Dockerfile's `ARG BASE_IMAGE` default (a folder that does not descend from a glass-box image, like `runtime-check`, builds standalone). Every image publishes for `linux/amd64` and `linux/arm64`; no image is arch-restricted. (The old per-challenge `LABEL org.glassbox.platforms` pin was removed: the binary rungs keep their exploitable binary x86-64 via `nbuild --arch` and run it under `qemu-user` on arm64, rather than restricting the container's platforms.)
...
- `platform/native/`: `FROM` harness; a `node:alpine` stage esbuilds a C-mode editor bundle (`@codemirror/lang-cpp` → `codemirror-bundle.js`) + a read-only `codemirror-c-view.js`; installs `gcc` + binutils, plus the x86-64 and aarch64 cross compilers, `qemu-user-static`, and `gdb-multiarch` (uniformly on both image variants) so a fixed-arch binary builds, runs, and is debugged on either host; and ships `nbuild` (the shared recompile helper: pick the compiler for the challenge's `--arch`, allowlist the flags, invoke it as an argv array with hook-owned `-o`/input, compile to a temp file, atomic `mv` on success), a C `lint.php` (`gcc -fsyntax-only` on the assembled unit, `#line`-mapped), a `native-run.php` (shared runner/introspection: sandboxed run under `qemu-user` when the host does not match the binary, payload decode, hexdump, `objdump`/`nm`/readelf-`checksec`, the stack-table renderer), and a `selftest.sh` that exercises the cross-arch run + capture path in-container by building an aarch64 fixture and driving it under `qemu`. It also ships `gdb`, used only by the Debug view to capture the real stack frame before and after the overflow, natively or, for a binary whose arch differs from the host, via `gdb-multiarch` over `qemu`'s gdbstub (best-effort, off the critical path); exploitation still happens locally against the downloadable binary. `checksec` is computed from readelf/nm (no external script). The debug dial applies here too: level-1 panels (byte calculator, your-bytes stack table, `checksec`, disassembly) versus level-2 panels (symbols/gadgets, memory map, program source, and the live `gdb`-captured before/after frame).
...
- **Linear git history, no merge commits.** Land work by rebasing the branch onto `main` (`git rebase main` in the worktree) and then fast-forwarding: `git merge --ff-only <branch>`. Never merge `main` into a feature branch to catch up, rebase instead; that is where this repo's one historical merge commit came from. The repo config enforces it (`merge.ff = only` refuses a non-fast-forward merge, `pull.rebase = true` makes `git pull` rebase), and GitHub enforces it on the other side: the repository's merge-commit option is off, so a pull request lands as a rebase or a squash. `git merge --no-ff` still overrides the config, so use it only when a merge commit is genuinely wanted.
...
- **Tooling is decided in the open, never excluded silently.** No tool is dropped from (or added to) the images or the contributor toolchain by fiat. When a tool is deliberately left out or brought in, state the choice and its rationale here; when proposing a tool change, surface the trade-offs (image size, offline/build cost, security, didactic value) to the maintainer for a decision rather than banning or admitting it silently.
...
- **README images live in `assets/` and are committed** (the deliberate exception to the rule above: a screenshot is neither fetched nor built, and GitHub can only render a file that is in the repo). Screenshots of a challenge UI are captured in both colour schemes and referenced through a `<picture>` element, must show no working payload, flag, or fix, and are re-captured when the interface they picture changes. Regenerate them with `assets/shots.mjs` (run the challenge container on a free port, then `node assets/shots.mjs`); it drives headless chromium over CDP in both schemes and pads and compresses the results. Nothing in CI checks them.
