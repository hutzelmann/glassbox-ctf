...
- **The debug switch (`?debug=1`).** A sticky toggle on every page reveals the
  internals that matter for *this* vulnerability. In the web challenges that is
  the literal SQL string your injection produced, the returned rows, the admin's
  rendered page, JS console errors. In the binary challenges it is a hexdump of
  the bytes the program received, the disassembly and symbol addresses, the
  `checksec` protections in force, a live stack-frame table, and the crash
  registers. You watch the bug work instead of guessing.
