...
protections, a hexdump of the bytes the server received, the terminating signal of
a crashing run, and a stack-frame view mapping the submitted bytes onto the
buffer, the saved frame pointer, and the saved return address. Where the host
permits capturing registers at the crash, the debug view SHALL also show the
instruction pointer the overflow produced. Guidance that interprets these
...
- **THEN** the stack-frame view marks which submitted bytes overwrote the saved
  return address and, where available, shows the resulting instruction pointer
