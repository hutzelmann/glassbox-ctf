...
## Purpose
Defines what the repository's front page owes a stranger: the pitch, a container
running before any theory, a catalog that stays in step with `challenges/`, and
imagery that shows the glass box without answering a challenge. `challenge-docs`
governs the per-challenge files; this governs the door they are behind.
## Requirements
